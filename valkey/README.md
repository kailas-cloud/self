...
- Установленный valkey-operator в кластере
- StorageClass `hcloud-volumes` (Hetzner Cloud Volumes)

## Установка оператора

```bash
kubectl apply -f valkey-operator/dist/install.yaml
```

## Деплой кластера

```bash
kubectl apply -k self/valkey/
```

## Конфигурация

### Valkey CR (`valkey-cluster.yaml`)
