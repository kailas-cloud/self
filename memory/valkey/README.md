...
```bash
kubectl apply -k self/memory/valkey
```

## Шаблон инстанса

Основной манифест кластера:
`self/memory/valkey/valkey.yaml`

Namespace для memory layer: `valkey`.

Если нужен отдельный инстанс под приложение — скопируйте папку `self/memory/valkey` и поменяйте `namespace`, `metadata.name` и ресурсы.
...
Настройки модулей задаются в `self/memory/valkey/valkey.conf` и входят в образ.
Образ подменяет `valkey.conf` и `ENTRYPOINT`, поэтому `*_MODULE_ARGS` не используются.

Актуальная строка загрузки:
```
loadmodule /usr/lib/valkey/libsearch.so --use-coordinator --reader-threads 8 --writer-threads 4 --hnsw-block-size 10000 --log-level notice
```
...
```bash
kubectl delete -k self/memory/valkey
```
